...
<div>
 <v-container align-content-center>
  ...
  <v-layout>
   <v-flex>
    <v-form ref="admin" lazy-validation v-on:submit.prevent>
     <v-card>
      <v-toolbar>
       <v-text-field id="txtsearch" @keyup.enter.native="searchAdminUsers()" v-model="q" hide-details single-line label="Email o nombre o apellido o telefono"></v-text-field>
       <v-btn icon @click="searchAdminUsers()">
   ...
   <v-flex>
    <v-card>
     <v-card-title class="headline grey lighten-2">
      ...
      <v-spacer></v-spacer>
      <v-progress-linear :indeterminate="processDataTableList" v-if="processDataTableList"></v-progress-linear>
     ...
     <v-data-table :rows-per-page-items="[5, 10, 15]" :items="users" class="elevation-1" :headers="headers" :pagination.sync="pagination" :total-items="total">
      <template slot="items" slot-scope="props">
       ...
       <td class="text-xs-center">{{ props.item.franchise.phone }}</td>
       <td class="text-xs-center">{{ props.item.branch.name }}</td>
  ...
  <v-dialog v-model="dialog" persistent max-width="400">
   ...
   <v-form ref="form" v-model="valid" lazy-validation>
    ...
    <v-card>
     ...
     <v-progress-linear :indeterminate="processChangePassword" v-if="processChangePassword"></v-progress-linear>
     <v-card-text>
     ...
     <v-container text-xs-center>
      <!-- <v-layout row wrap align-center>
       <v-flex>
        <v-text-field label="Nombre" disabled v-model="user.first_name"></v-text-field>

        <v-text-field label="Apellido" disabled v-model="user.last_name"></v-text-field>
       </v-flex>
      </v-layout>-->
...
export default {
 ...
 methods: {
  searchAdminUsers() {
   ...
   //destructurar objectos, sacar solo lo que necesitamos de objeto
   const {
    sortBy,
    descending,
    page,
   ...
   this.processDataTableList = true
   vm.axios.get(listAdminUsersUrl + '?page=' + vm.pagination.page + '&rowsPerPage=' + vm.pagination.rowsPerPage + '&q=' + vm.q, {
     headers: getHeaders()
    })
    .then(response => {
     vm.processDataTableList = false
  ...
  resetPassword() {
   ...
   if (vm.$refs.form.validate()) {
    ...
    vm.processChangePassword = true

    vm.axios({

     url: adminUserChangePasswordUrl,
     method: 'POST',
     data: vm.user,
     headers: getHeaders()
     ...
     vm.processChangePassword = false
    })
   }
  },
